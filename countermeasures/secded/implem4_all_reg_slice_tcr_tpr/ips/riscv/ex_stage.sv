...
`ifdef DIFT
  ...
  logic        check_b;
  logic        rs1_o_tag;
  logic        rs1_o_tag_protec;
  logic [1:0]  rs1_o_tag_hc;
  logic        rs1_o_tag_sd;
  logic        sec_interrupt;
  logic        ded_interrupt;
`endif
...
begin
  if (ex_valid_o) begin
    if (regfile_we_i) begin
      rs1_o_tag_protec = alu_operand_a_i_tag;
    end else begin 
        rs1_o_tag_protec = rs1_o_tag_corr;
      end
  end else begin 
    rs1_o_tag_protec = rs1_o_tag_corr;
  end
  ...
  // -----------------------------------------------------------------------------------------
  riscv_secded_encoder #(.REG_SIZE(1), .HC_SIZE(2), .G_PARITY(G_PARITY)) secded_encoder_rs1_o_tag (
    .clk          ( clk               ),
    .reg_i        ( rs1_o_tag_protec  ),
    .sd_o         ( rs1_o_tag_sd      ),
    .hc_o         ( rs1_o_tag_hc      )
  );

  riscv_secded_decoder #(.REG_SIZE(1), .HC_SIZE(2), .G_PARITY(G_PARITY)) secded_decoder_rs1_o_tag (
    .sd_i           ( rs1_o_tag_sd    ),
    .hc_i           ( rs1_o_tag_hc    ),
    .reg_i          ( rs1_o_tag       ),
    .reg_o_corr     ( rs1_o_tag_corr  ),
