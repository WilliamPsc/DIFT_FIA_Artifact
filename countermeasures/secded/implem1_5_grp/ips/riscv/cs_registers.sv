...
`ifdef DIFT
  ...
  logic [4:0]  tpr_hc;
  logic [17:0] tpr_sl_corr;
  logic        tpr_sd;
  logic [31:0] tcr_q, tcr_n;
  logic [4:0]  tcr_hc;
  logic [21:0] tcr_sl_corr;
  logic        tcr_sd;
...
`endif
  ...
  `ifdef DIFT
    // ------------------------------------------------------------------------------------------
    riscv_secded_encoder #(.REG_SIZE(18), .HC_SIZE(5), .G_PARITY(G_PARITY)) secded_encoder_tpr (
      .clk    ( clk         ),
      .reg_i  ( tpr_n[17:0] ),
      .sd_o   ( tpr_sd      ),
      .hc_o   ( tpr_hc      )
    ...
    riscv_secded_decoder #(.REG_SIZE(18), .HC_SIZE(5), .G_PARITY(G_PARITY)) secded_decoder_tpr (
      .reg_i          ( tpr_q[17:0]     ),
      .sd_i           ( tpr_sd          ),
      .hc_i           ( tpr_hc          ),
      .reg_o_corr     ( tpr_sl_corr     ),
      .sec_interrupt  ( sec_interrupt_0 ),
      .ded_interrupt  ( ded_interrupt_0 )
    );
    assign tpr_o_corr = {14'b0, tpr_sl_corr};
    
    // ------------------------------------------------------------------------------------------
    riscv_secded_encoder #(.REG_SIZE(22), .HC_SIZE(5), .G_PARITY(G_PARITY)) secded_encoder_tcr (
      .clk    ( clk         ),
      .reg_i  ( tcr_n[21:0] ),
      .sd_o   ( tcr_sd      ),
      .hc_o   ( tcr_hc      )
    ...
    riscv_secded_decoder #(.REG_SIZE(22), .HC_SIZE(5), .G_PARITY(G_PARITY)) secded_decoder_tcr (
      .reg_i          ( tcr_q[21:0]     ),
      .sd_i           ( tcr_sd          ),
      .hc_i           ( tcr_hc          ),
      .reg_o_corr     ( tcr_sl_corr     ),
      .sec_interrupt  ( sec_interrupt_1 ),
      .ded_interrupt  ( ded_interrupt_1 )
    );
    assign tcr_o_corr = {10'b0, tcr_sl_corr};
